...
void DirectLightingIntegrator::Li(const Scene *scene,
        const Renderer *renderer, const RayDifferential *ray,
        const Intersection *isect, const Sample *sample, RNG &rng, MemoryArena &arena,
   ...
   for ( size_t i = 0; i < count; i++){
    ...
    if (ray[i].depth + 1 < maxDepth) {
        Vector wi;
        // Trace rays for specular reflection and refraction
        L[i] += SpecularReflect(ray[i], bsdf, rng, isect[i], renderer, scene, &sample[i], arena);
        L[i] += SpecularTransmit(ray[i], bsdf, rng, isect[i], renderer, scene, &sample[i], arena);
    }
    }
